...
const ServiceTypeSelect = ({ value, onChange, optionsMap }) => {
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      <button type="button" onClick={() => setIsOpen(!isOpen)} className={`flex h-8 w-full items-center justify-between rounded-lg border bg-white px-3 text-sm-text font-medium transition-all focus:outline-none ${isOpen ? "border-gray-900  ring-gray-900" : "border-slate-200 hover:border-gray-300"}`}>
      ...
      {isOpen && (
        <div className="absolute custom-scrollbar left-0 mt-1 w-full bg-white border border-gray-100 rounded-lg shadow-xl z-[999] overflow-hidden py-1 max-h-60 overflow-y-auto">
          {Object.entries(optionsMap).map(([key, label]) => (
            <button key={key} type="button" onClick={() => { onChange(key); setIsOpen(false); }} className="w-full px-3 py-2 mb-1 text-left text-sm-text flex items-center justify-between transition-colors hover:bg-gray-50 font-medium text-slate-700 ">
...
export default function ServiceCard({ service, isEditing, tempData, setTempData, onEdit, onSave, onCancel, onToggle, onDelete }) {
    ...
    <div className="w-full rounded-xl border border-app-dark/20 bg-app-light p-4 shadow-md animate-in fade-in slide-in-from-top-2 relative z-0">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-h3 font-medium text-gray-900  ">Delete this service?</h3>
            <p className="text-sm-text text-red-700 font-bold">This will permanently remove {service.name}.</p>
          </div>
        </div>

        <div className="flex gap-2 w-full md:w-auto mt-2 md:mt-0">
          <Button 
            className="flex-1 md:w-32 !bg-red-600 text-white text-micro font-medium   h-9 shadow-sm active:scale-95 transition-transform" 
          ...
          <Button 
            variant="outline" 
            className="flex-1 !text-text-dark md:w-24 border-1 !border-app-dark text-micro font-medium   h-9 active:scale-95 transition-transform" 
  ...
  if (isEditing) {
    return (
      <div className={`${containerBase} border-app-dark/70 bg-white shadow-md animate-in fade-in zoom-in-95 py-3 pt-2 relative z-20`}>
        <div className="flex flex-col md:flex-row justify-between items-center gap-3">
          <div className="flex-1 w-full space-y-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-3 gap-y-1">
              <div className="space-y-1">
                <label className="text-micro font-medium text-text-dark/60  ml-1 ">Service Name</label>
                <Input value={tempData?.name || ""} onChange={(e) => setTempData({...tempData, name: e.target.value})} className="text-sm-text font-medium h-8" />
              </div>
              ...
              <div className="space-y-1 relative z-10">
                ...
                <Input type="number" value={tempData?.price_per_kg || ""} onChange={(e) => setTempData({...tempData, price_per_kg: e.target.value})} className="text-sm-text font-medium h-8" />
              </div>
            </div>
          </div>
          <div className="flex flex-row md:flex-col gap-1.5 w-full md:w-auto relative z-0">
            <Button variant="success" className="h-8 flex-1 md:w-28 text-micro font-medium   bg-green-700 text-white hover:bg-green-800" onClick={() => handleAction(onSave, service.id, tempData)} disabled={isProcessing}>
            ...
            <Button variant="outline" className="h-8 flex-1 md:w-28 text-micro font-medium  " onClick={onCancel} disabled={isProcessing}>
    ...
    <div className={`${containerBase} ${service.is_active ? 'bg-white border-slate-200' : 'bg-slate-50 border-slate-200'} !py-4 flex items-center relative z-0`}>
      <div className="flex flex-col md:flex-row justify-between items-center w-full gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                ...
                <Badge className="bg-white text-gray-600 border-gray-200 px-2 font-mono  font-bold py-0.5">₱{Number(service.price_per_kg).toFixed(2)}</Badge>
        ...
        <div className="flex !flex-row md:flex-col items-center justify-end gap-2 shrink-0 ml-auto">
          {service.is_active && (
            <Button variant="outline" className="h-8 flex-1 md:w-28 text-micro font-medium   px-3 border-gray-100 hover:bg-app-dark/5" onClick={() => onEdit(service)}>
          ...
          <div className="flex flex-row gap-2 justify-end">
            {!service.is_active && (
              <Button 
                ...
                className="h-8 px-2.5 bg-transparent text-red-600 shrink-0 hover:bg-transparent hover:opacity-100 transition-none"
              >
            ...
            <Button 
              ...
              className={`
                h-8 flex-1 md:w-28 text-micro font-medium  px-3 transition-all active:scale-95
                ${service.is_active 
