...
const CancelledOrderDetailModal = ({ order, isOpen, onClose }) => {
  ...
  // Reset privacy and manage body scroll / escape key
  useEffect(() => {
  ...
  {isOpen && (
    ...
    >
      ...
      >
        {/* HEADER: Matching Unclaimed Style */}
        <div className="bg-rose-50/50 pt-5 pb-3 px-4 text-center relative border-b border-rose-100/50 shrink-0">
          ...
          <div className="flex flex-wrap items-center justify-center gap-1.5 mt-1 text-micro font-medium uppercase tracking-wider text-text-dark/70">
            ...
            <span>•</span>
            <span className="text-rose-500 font-bold">CANCELLED ARCHIVE</span>
          </div>
        </div>
        ...
        <div className="overflow-y-auto custom-scrollbar flex-1 p-5 space-y-3">
          ...
          {/* PII Protection Box (Phone & Address) */}
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-[11px] font-bold tracking-wider text-rose-600 uppercase">
                  {order.is_paid ? `${order.payment_method}` : "Unpaid"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50 border border-slate-200/60 p-4 rounded-2xl space-y-3">
            ...
            <div className="grid grid-cols-1 gap-1.5 text-micro font-medium tracking-tight text-text-dark/60">
              <div className="flex justify-between">
                <span>Created:</span>
                <span className="text-text-dark font-bold">{createdDateTime}</span>
              </div>
              <div className="flex justify-between">
                <span>Cancelled:</span>
                <span className="text-rose-600 font-bold">{cancelledDateTime}</span>
              </div>
            </div>
          </div>
...
const CancelledOrderCard = ({ order, onOpenDetail }) => {
  ...
  const timeString = order.cancelled_at_date instanceof Date && !isNaN(order.cancelled_at_date)
    ? order.cancelled_at_date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })
    : "--:--";
