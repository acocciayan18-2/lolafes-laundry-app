...
export default function TodayOrders({ orders = [], isLoading }) {
  ...
  const executeStatusUpdateCall = useCallback(async (newStatus, sendSms = false) => {
    ...
    // ✨ FIX 1: Destructure handover_method from the selected order
    const { customer_phone, customer_name, order_number, is_walk_in, handover_method } = selectedOrder;
    ...
    try {
      ...
      if (success) {
        ...
        // ✨ FIX 2: Double-check that it is NOT a walk-in before opening the SMS gateway
        if (sendSms && !is_walk_in && customer_phone) {
          import('../../services/smsService')
            .then(s => s.sendStatusSMS(customer_phone, customer_name, order_number,  handover_method))
