...
import { useActivityStore } from "../../store/activities/useActivityStore"; // ✨ Added as requested
import {
...
// DATA GUARD: Ensure robust date parsing
...
export default function TodayOrders({ orders = [], isLoading }) {
  ...
  // Included as requested, though logic implementation depends on your exact use case
            ...
            >
              ...
              <div className="bg-slate-50/80 pt-5 pb-3 px-4 text-center relative border-b border-slate-100 shrink-0">
                <button 
                  ...
                  disabled={isUpdating} 
                  className="absolute top-4 right-4 p-2 opacity-40 hover:opacity-100 transition-opacity focus:outline-none focus:ring-1 focus:ring-app-dark rounded-lg"
                >
                  <IconClose className="w-5 h-5 text-text-dark" />
                </button>
                <div className="w-10 h-10 flex items-center justify-center mx-auto mb-1">
                   <IconShirt className="w-6 h-6 text-app-dark" />
                </div>
                  ...
                  {Object.entries(statusConfig).filter(([k]) => k !== 'pending' && k !== 'delivered').map(([key, cfg]) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-sm-text font-medium uppercase flex-1 text-left tracking-wide">
