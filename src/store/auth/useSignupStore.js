...
export const useSignupStore = create((set, get) => ({
  ...
  verifyAndSignup: async (email, password, enteredOtp, role, onSuccessCallback) => {
    const state = get();
    if (state.isLoading) return false;
    ...
    try {
      ...
      try {
        await setDoc(doc(db, "users", userCredential.user.uid), {
          email: cleanEmail,
          role: role || "STAFF", // Defaults to STAFF if undefined
          created_at: new Date().toISOString()
        });
      } catch (dbError) {
        console.error("Warning: Could not save user role to database:", dbError);
      }
