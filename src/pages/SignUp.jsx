    ...
    <div className="flex justify-center items-center w-full min-h-screen p-3 bg-white overflow-y-auto">
      ...
      <main className="flex flex-col w-full max-w-[350px] p-3 bg-white mt-[-100px]">
        <header className="flex justify-center items-center w-full mb-3 flex-col">
          <div className="flex justify-center items-center w-14 h-14 bg-app-dark rounded-xl overflow-hidden mb-2">
            <img src="/images/lolafeslaundry-logo-transparent.png" alt="Lola Fe's Laundry Logo" className="w-12 h-12 object-contain" />
          </div>
          <h1 className="text-center text-h1 font-bold text-text-dark mb-1">Create Admin Account</h1>
        ...
        <form onSubmit={handleSubmit} noValidate aria-label="Sign up form">
          ...
          <div className="mb-3 text-start w-full">
            ...
            <div className="relative flex items-center w-full">
              ...
              <button
                type="button"
                className="absolute right-3 p-1 rounded hover:bg-slate-100 transition-colors focus:ring-1 focus:ring-app-dark outline-none disabled:opacity-50"
          ...
          {isOtpSent && (
            <div className="mb-3 animate-fade-in" role="region" aria-label="OTP Verification">
              ...
              <input
                ...
                pattern="[0-9]*"  
                id="otp"
                className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-1 focus:ring-app-dark outline-none text-center tracking-widest text-lg font-bold transition-all disabled:opacity-50 disabled:bg-slate-50"
                placeholder="000000"
                value={enteredOtp}
          ...
          {isOtpSent && (
            <div className="text-center mb-1">
              <button
                ...
                disabled={isLoading}
                className="text-micro text-text-dark/70 font-normal hover:text-text-dark focus:text-text-dark transition-colors outline-none rounded focus:ring-1 focus:ring-slate-200 disabled:opacity-50"
              >
                Cancel
              </button>
            </div>
        ...
        <footer className="mt-3 text-center flex gap-3 justify-center">
          <p className="text-micro text-text-dark/70">
            Already have an account?{" "}
            <Link
              to={isLoading ? "#" : "/login"}
              className={`text-micro text-blue-600 font-bold no-underline rounded focus:outline-none focus:ring-1 focus:ring-blue-200 ${isLoading ? "opacity-50 cursor-not-allowed pointer-events-none" : "hover:text-blue-800"}`}
            >
              Login
            </Link>
          </p>
